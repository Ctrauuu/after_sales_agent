...
聚合服务位于 [`backend/mcp_suning/order_timeline_server.py`](../backend/mcp_suning/order_timeline_server.py)，纯数据合并逻辑位于 [`backend/mcp_suning/order_timeline.py`](../backend/mcp_suning/order_timeline.py)。
...
`TimelineMCPGateway` 的工具名和端点是代码固定的，不接受模型指定 URL、工具名或身份字段。相关实现见 [`timeline_client.py`](../backend/mcp_suning/timeline_client.py) 和 [`auth_attestation.py`](../backend/mcp_suning/auth_attestation.py)。
...
`SLAConfig` 定义在 `order_timeline.py`，单位均为小时：
